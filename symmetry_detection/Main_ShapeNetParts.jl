...
@everywhere function main(modelname)
	for partnum = 0:1000
	    ...
	    println(filename)
	    newMesh, mcenter, radius = loadMesh_v2(filename)
	    ...
	    close(fout)
	    symname = "/orions3-zfs/projects/anastasiad/ShapeNetSymm/Results/03001627/" * modelname * "_$partnum.sym4"
	    #println(symname)
	    saveSymmetry(symname, symType, translate, canonical, mcenter, radius)
    end
end
